...
def executeStatus ():
    ...
    SELECT
        ...
        WHERE
            es.eqstag IN ('RT_ENT_EQUIP_STATUS_INSTALLED')) as A"""
    ...
    redshiftQuery = """SELECT 
                ...
                WHERE vehicle IN ({0})
                AND ft_trip_report_agg.dt_begin_trip between '{1}' and '{2}'
                """.format(vehicleListString, dateFrom, dateTo)
    ...
    lastObjectDate = finalVehicleDataFrame.select("dt_end_trip").agg(max("dt_end_trip").alias("dt_end_trip")).collect()[0]['dt_end_trip']
    
    if lastObjectDate == None:
        lastObjectDate = dateFrom
    
    ### Query to get the details data regarding the trips in the range date ###
    redshiftQuery = """SELECT DISTINCT VEHICLE AS VEHICLE_DETAILED,
                        ...
                        WHERE VEHICLE IN ({0})
                        AND  DT_BEGIN_TRIP BETWEEN '{1}' AND '{2}'""".format(vehicleListString, dateFrom, dateTo)
    ...
    ## Gerando o CSV do dataframe inteiro
    #tripReportPartitioned.coalesce(1).write.mode("overwrite").format('csv').options(header=True).save(bucketUrl+fileName)
    
    s3TempDir = awsconf.REDSHIFT_S3_TEMP_OUTPUT
    redshiftURL = dbconf.REDSHIFT_DW_SASCAR_URL
    
    ### Make a connection to Redshift and write the data into the table 'stage.STATUS' ###
    tripReportPartitioned.write \
        .format("com.databricks.spark.redshift") \
        .option("url", redshiftURL) \
        .option("dbtable", "stage.STATUS") \
